...
for key in all_bom_components_lic_info.keys():
    #print out a part of the license details
    for each_license_display in all_bom_components_lic_info[key]:
        #print out the license info only if it's receiprocal, weak reciprocal, AGPL, unknown or proprietary
        #if the length of each_license_display['licenses'] is 0, then just print license family name
        if len(each_license_display['licenses']) == 0 and each_license_display['licenseFamilyName'] in ('RECIPROCAL', 'WEAK_RECIPROCAL', 'UNKOWN', 'AGPL'):
            #print single license details
                print("\nComponent: \t\t\t" + key)
                #print("License Display: \t" + each_license_display['licenseDisplay'])
                print("License Name: \t\t" + each_license_display['licenseDisplay'])
                print("License Family: \t" + each_license_display['licenseFamilyName'])
        else:
            flag = 0
            for each_license in each_license_display['licenses']:
                if each_license['licenseFamilyName'] in ('RECIPROCAL', 'WEAK_RECIPROCAL', 'UNKOWN', 'AGPL'):
                    if flag == 0:
                        print("\nComponent: \t\t\t" + key)
                        #print("License Display: \t" + each_license_display['licenseDisplay'])
                        flag = 1
                    print("License Name: \t\t" + each_license['licenseDisplay'])
                    print("License Family: \t" + each_license['licenseFamilyName'])
                else:
                    continue
print()
